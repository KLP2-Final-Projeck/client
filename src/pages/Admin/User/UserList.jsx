...
const UserList = () => {
    ...
    const deleteUser = async (id) => {
        try {
            await axios.delete(`http://localhost:4002/user/${id}`)
            getUsers();
        } catch (error) {
            console.log(error);
        }
    ...
    return (
        <>
            ...
            <div className="container columns mt-5">
                ...
                <button className='btn btn-primary d-flex justify-content-start mb-3' onClick={() => { navigate('/admin/add') }}>Add User</button>
                            ...
                            {users.map((user, index) => (
                                <tr key={user.id}>
                                    ...
                                    <td>
                                        ...
                                        <Link to={`/admin/edit/${user.id}`} className='btn btn-warning mb-2 bt'>Update</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
